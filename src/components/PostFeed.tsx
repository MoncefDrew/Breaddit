...
const PostFeed: FC<PostFeedProps> = ({ initialPosts, subredditName }) => {
  //referncing the last post in the view port to observe it
  const lastPageRef = useRef<HTMLElement>();
  const { ref, entry } = useIntersection({
    root: lastPageRef.current,
    threshold: 1,
  });

  //using session this way cus we're inside a client component
  const { data: session } = useSession();

  //fetching the posts using useInfiniteQuery
